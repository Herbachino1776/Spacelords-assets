/* ====== Spacelords SoR Demo — GitHub Pages (relative assets + exact name lists) ====== */
...
var HUD = { lines:[], add:function(s){ this.lines.push(String(s)); if(this.lines.length>12) this.lines.shift(); } };
...
function hudText(){
  ...
  ctx.fillStyle='rgba(0,0,0,.45)'; ctx.fillRect(6,6,380,14*(HUD.lines.length+1));
...
function defaultAnims(cat, frames){
  ...
  var m={};
  if(cat==='player' || cat==='npc'){ m.idle=mk(0,4,DUR.idle); m.walk=mk(4,10,DUR.walk); m.jab=mk(10,12,DUR.jab); m.hook=mk(12,16,DUR.hook); m.hit=mk(16,18,DUR.hit); }
...
function loadSet(category, name, spec){
  ...
  return Promise.all([loadImage(sheetURL), fetchJSON(manifestURL)]).then(function(arr){
    ...
    return {sheet:sheet, anims:anims, spec:spec, name:name, category:category};
  });
}
...
/* ===== NPCs ===== */
function NPC(sheet,anims){ Sprite.call(this,sheet,anims,2); this.speed=0.6; this.dir=Math.random()<0.5?-1:1; }
NPC.prototype=Object.create(Sprite.prototype);
NPC.prototype.ai=function(dt,threats){
  var near=false;
  for (var i=0;i<threats.length;i++) if (Math.abs(threats[i].x - this.x) < 80) near=true;
  if (near){ this.dir = -this.dir; this.set('walk'); }
  this.x += this.dir * this.speed * dt * 0.15;
  if (this.x<16){ this.x=16; this.dir=1; } if (this.x>W-16){ this.x=W-16; this.dir=-1; }
  this.flip = this.dir<0;
  if (!near) this.set('idle');
};
...
var game={ state:STATE.LOCKED, stage:1, score:0, player:null, enemies:[], npcs:[], pickups:[], crates:[], boss:null, shift:0 };

/* ===== Exact names from your last zip ===== */
var LIST_PLAYER  = ["MantidPrime","Scarabos"];
var LIST_SMALLS  = ["Stardust","Celestis"];
var LIST_LARGES  = ["OrionLord"];
var LIST_WEAPONS = ["CosmicStaff","StingerBlade"];
var LIST_NPCS    = ["Oracle","LarvalKid"];
var ITEM_NAME    = "CosmicLoot";

/* ===== Boot: load everything that exists, then run ===== */
(function boot(){
  drawBG(0); HUD.add("Loading assets from "+ASSET_BASE+" …");

  var playerSet=null, smallSets=[], largeSets=[], weaponSets=[], npcSets=[], itemSet=null;
  ...
  function loadNPC(name, cb){ loadSet('npc', name, SPEC.npc).then(function(s){ if(s){ HUD.add("npc    ✓ "+name); npcSets.push(s);} cb(s); }); }

  seqLoad(LIST_PLAYER, loadPlayer, function(){
    seqLoad(LIST_SMALLS, loadSmall, function(){
      seqLoad(LIST_LARGES, loadLarge, function(){
        seqLoad(LIST_WEAPONS, loadWeapon, function(){
          seqLoad(LIST_NPCS, loadNPC, function(){
  ...
  function start(){
    ...
    function populateNPCs(){
      for (var i=0;i<Math.min(3, Math.max(1, npcSets.length)); i++){
        var ns = npcSets[i % Math.max(1, npcSets.length)];
        var n = new NPC(ns.sheet, ns.anims);
        n.x = rand(80, W-80); n.y=300; n.set('idle');
        game.npcs.push(n);
      }
    }
    ...
    function update(dt){
      ...
      for (var j=0;j<game.npcs.length;j++){ var n=game.npcs[j]; n.ai(dt, game.enemies); n.update(dt); }
    ...
    function draw(){
      ...
      for (var j=0;j<game.npcs.length;j++) game.npcs[j].draw(ctx);
      for (var k=0;k<game.enemies.length;k++) game.enemies[k].draw(ctx);

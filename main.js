/* ====== Spacelords SoR Demo — GitHub Pages build ====== */
/* Primary base: your GitHub Pages assets repo. RAW is used as an automatic fallback if needed. */
var PAGES_BASE = "https://herbachino1776.github.io/Spacelords-assets/assets";
var RAW_BASE   = "https://raw.githubusercontent.com/Herbachino1776/Spacelords-assets/29adca886f3339e47528fce7ea895acfcb3457c4/assets";
var ASSET_BASE = null;
...
var HUD = { lines:[], add:function(s){ this.lines.push(String(s)); if(this.lines.length>8) this.lines.shift(); } };
...
function hudText(){
  ...
  ctx.fillStyle='rgba(0,0,0,.45)'; ctx.fillRect(6,6,300,14*(HUD.lines.length+1));
...
/* ===== Specs ===== */
var SPEC = {
  player:{fw:96,fh:96,cols:6,rows:4},
  enemy_small:{fw:80,fh:80,cols:6,rows:3},
  enemy_large:{fw:120,fh:120,cols:6,rows:4},
  weapons:{fw:96,fh:48,cols:6,rows:2},
  items:{fw:48,fh:48,cols:8,rows:2}
};
...
function defaultAnims(cat, frames){
  ...
  else if(cat==='enemy_large'){ m.idle=mk(0,4,DUR.idle); m.walk=mk(4,10,DUR.walk); m.attack=mk(10,14,DUR.attack); m.hit=mk(14,16,DUR.hit);
    m.knockdown=mk(16,18,DUR.knockdown); m.rise=mk(18,20,DUR.rise); m.death=mk(20,24,DUR.death); }
...
(function chooseBase(){
  drawBG(0);
  HUD.add("Probing GitHub Pages…");
  loadImage(PAGES_BASE + "/sprites/enemy_large/OrionLord/OrionLord_sheet.png").then(function(img){
    if (img){ ASSET_BASE=PAGES_BASE; HUD.add("Base: GitHub Pages ✓"); loadGame(); }
    else {
      HUD.add("Pages failed, trying RAW…");
      loadImage(RAW_BASE + "/sprites/enemy_large/OrionLord/OrionLord_sheet.png").then(function(img2){
        if (img2){ ASSET_BASE=RAW_BASE; HUD.add("Base: RAW commit ✓"); loadGame(); }
        else { HUD.add("Assets not reachable. Check Pages/paths."); banner("Asset base not reachable"); }
      });
    }
  });
})();
...
function loadGame(){
  ...
  var PLAYER_CAND=["MantidPrime","Scarabos"];
  var SMALLS=["Celestis","Stardust"];
  var BOSS_NAME="OrionLord";
  var WEAPONS=["StingerBlade","CosmicStaff"];
  var ITEM_SET="CosmicLoot";

  var playerSet=null, smallSets=[], bossSet=null, weaponSets=[], itemSheet=null;

  function loadPlayers(i){ if(i>=PLAYER_CAND.length) return loadSmalls(0);
    loadSet('player', PLAYER_CAND[i], SPEC.player).then(function(s){ if(s&&!playerSet) playerSet=s; loadPlayers(i+1); }); }
  function loadSmalls(i){ if(i>=SMALLS.length) return loadBoss();
    loadSet('enemy_small', SMALLS[i], SPEC.enemy_small).then(function(s){ if(s) smallSets.push(s); loadSmalls(i+1); }); }
  function loadBoss(){ loadSet('enemy_large', BOSS_NAME, SPEC.enemy_large).then(function(s){ bossSet=s; loadWeapons(0); }); }
  function loadWeapons(i){ if(i>=WEAPONS.length) return loadItems();
    loadSet('weapons', WEAPONS[i], SPEC.weapons).then(function(s){ if(s) weaponSets.push(s); loadWeapons(i+1); }); }
  function loadItems(){ loadImage(ASSET_BASE + "/sprites/items/" + ITEM_SET + "/" + ITEM_SET + "_sheet.png").then(function(img){ itemSheet=img; start(); }); }

  loadPlayers(0);

  function start(){
    if (!playerSet){ drawBG(0); banner("Player sheet not found"); HUD.add("Upload player sheet (e.g., MantidPrime)"); return; }
    ...
    function randomItemIcon(){
      ...
      var fw=48, fh=48, cols=8; var idx = Math.floor(Math.random()*16);
      var c = idx % cols, r = (idx/cols)|0;
      return { sheet:itemSheet, x:c*fw, y:r*fh, w:fw, h:fh };
    }
    function spawnSmallWave(n){
      for (var i=0;i<n;i++){
        if (!smallSets.length) break;
        var sp=smallSets[Math.floor(Math.random()*smallSets.length)];
    ...
    function spawnBoss(){
      if (!bossSet) return;
      var b=new Boss(bossSet.sheet, bossSet.anims); b.x=W+60; b.y=300; b.flip=true; game.boss=b; game.enemies.push(b);
    }
    ...
    function update(dt){
      game.shift += (game.state===STATE.TRANSITION ? dt*0.05 : 0);
      ...
      for (var p=game.pickups.length-1;p>=0;p--){
        ...
        if (overlap(playerHB, pp.box())){
          ...
          if (pp.kind==='weapon' && weaponSets.length){
            var w=weaponSets[Math.floor(Math.random()*weaponSets.length)];
            game.player.weapon = w.name; game.player.wSet = w;
            (function(pl){ setTimeout(function(){ pl.weapon=null; pl.wSet=null; }, 15000); })(game.player);
          }
      ...
      // Waves → boss → win
      if (game.enemies.length===0 && !game.boss && game.state!==STATE.TRANSITION){
        if (game.stage % 3 === 0) { spawnBoss(); game.state=STATE.BOSS; }
        else {
          game.state=STATE.TRANSITION;
          setTimeout(function(){
            game.stage++;
            game.crates.push(new Crate(rand(120,520),304));
            spawnSmallWave(4+Math.min(8,game.stage));
            game.state=STATE.LOCKED;
          }, 1200);
        }
      }

/* ====== Spacelords SoR Demo — GitHub Pages build (auto-fallbacks) ====== */
...
/* ===== Load a spriteset (sheet + optional manifest) ===== */
...
/* ===== Boot: choose base, load assets with aggressive fallbacks ===== */
(function chooseBase(){
  drawBG(0);
...
function loadGame(){
  ...
  function tryLoadPlayer(i){
    if (i>=PLAYER_CAND.length) return tryLoadSmalls(0);
    loadSet('player', PLAYER_CAND[i], SPEC.player).then(function(s){
      if (s && !playerSet) { playerSet=s; HUD.add("Player: "+s.name); }
      tryLoadPlayer(i+1);
    });
  }
  function tryLoadSmalls(i){
    if (i>=SMALLS.length) return loadBoss();
    loadSet('enemy_small', SMALLS[i], SPEC.enemy_small).then(function(s){
      if (s){ smallSets.push(s); HUD.add("Small enemy: "+s.name); }
      tryLoadSmalls(i+1);
    });
  }
  function loadBoss(){
    loadSet('enemy_large', BOSS_NAME, SPEC.enemy_large).then(function(s){ bossSet=s; if (s) HUD.add("Boss: "+s.name); loadWeapons(0); });
  }
  function loadWeapons(i){
    if (i>=WEAPONS.length) return loadItems();
    loadSet('weapons', WEAPONS[i], SPEC.weapons).then(function(s){ if (s){ weaponSets.push(s); HUD.add("Weapon: "+s.name); } loadWeapons(i+1); });
  }
  function loadItems(){
    loadImage(ASSET_BASE + "/sprites/items/" + ITEM_SET + "/" + ITEM_SET + "_sheet.png").then(function(img){ itemSheet=img; if (img) HUD.add("Items: "+ITEM_SET); start(); });
  }
  tryLoadPlayer(0);

  function start(){
    /* ---- Aggressive fallbacks so sprites ALWAYS show ---- */
    if (!playerSet && bossSet){
      // Use the boss sheet as the player (works surprisingly well)
      HUD.add("Fallback: using "+bossSet.name+" as player");
      playerSet = { sheet: bossSet.sheet, anims: bossSet.anims, spec: bossSet.spec, name: bossSet.name+"(as player)" };
    }
    if (!playerSet){
      banner("No sprites found — need at least one sheet.");
      HUD.add("Upload a player or OrionLord; refresh.");
      return;
    }
    if (!smallSets.length && bossSet){
      HUD.add("No small enemies — cloning boss as minions");
      // Create 2 clones referencing the boss sheet/anim for variety
      smallSets.push({ sheet: bossSet.sheet, anims: bossSet.anims, spec: bossSet.spec, name: bossSet.name+"MinionA" });
      smallSets.push({ sheet: bossSet.sheet, anims: bossSet.anims, spec: bossSet.spec, name: bossSet.name+"MinionB" });
    }
    ...
    // If we only have boss (used as player), still spawn boss after 1 wave
    spawnSmallWave(smallSets.length ? 4 : 0);
    if (!smallSets.length && bossSet) spawnBoss();
    ...
    function update(dt){
      ...
      for (var p=game.pickups.length-1;p>=0;p--){
        ...
        if (overlap(playerHB, pp.box())){
          ...
          if (pp.kind==='weapon' && false){ /* weapons optional; enable when sheets present */ }
      ...
      if (game.enemies.length===0 && !game.boss && bossSet){
        spawnBoss();
      }
